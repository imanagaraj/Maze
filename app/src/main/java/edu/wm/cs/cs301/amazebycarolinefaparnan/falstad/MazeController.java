...
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.Factory;
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.MazeConfiguration;
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.MazeContainer;
...
import edu.wm.cs.cs301.amazebycarolinefaparnan.ui.GeneratingActivity;
import edu.wm.cs.cs301.amazebycarolinefaparnan.ui.PlayActivity;

/**
 * Created by Aparna on 11/16/16.
 */
...
public class MazeController extends Application implements Order {
    ...
    public MazeController(){
        ...
        //panel = new GraphicsWrapper(this);
       // panel = new GraphicsWrapper(this);
    }

    public void setGraphics(MazePanel gw){
        System.out.println("Graphics are set");
        this.panel = gw;
    }
...
//	}
    ...
    public void setMaze(String driver1, String algorithm, int skill) {
        // Update driver based on user input
        robot = new BasicRobot();
        robot.setMaze(this);
        //this.driver = driver;
        //this.skill = skill;
        if(driver1.equals("Manual Driver")){
            driver = new ManualDriver();
            ((ManualDriver) driver).setRobot(robot);
            this.updateDriver(driver);
        }
        else if (driver1.equals("Wall Follower")){
            driver = new WallFollower();
            ((WallFollower) driver).setRobot(robot);
            this.updateDriver(driver);
        }
        else if (driver1.equals("Wizard")){
            driver = new Wizard();
            ((Wizard) driver).setRobot(robot);
            this.updateDriver(driver);
        }
        else if (driver1.equals("Pledge")){
            driver = new Pledge();
            ((Pledge) driver).setRobot(robot);
            this.updateDriver(driver);
        }

        // Update maze builder based on user input
        if(algorithm.equals("Default")){
            this.updateBuilder(Builder.DFS);
        }
        else if(algorithm.equals("Eller")){
            this.updateBuilder(Builder.Eller);
        }
        else if(algorithm.equals("Prim")){
            this.updateBuilder(Builder.Prim);
        }

        // Update skill level based on user input
...
//            System.out.println("SWITCHED TO PLAYING SCREEN");
...
////                    Constants.STEP_SIZE, seencells, mazeConfig.getRootnode(), this)) ;
////
...
//        }
        ...
        public void notifyViewerRedraw() {
            ...
            System.out.println("NOTIFY VIEW REDRAW");


            this.panel = PlayActivity.view;
            ...
            if(state != StateGUI.STATE_PLAY){
                System.out.println("state does not equal state play");
                return;
            }
//            Iterator<Viewer> it = views.iterator() ;
//            while (it.hasNext()) {
...
//            }
            ...
            rset = new RangeSet();
            map.redraw(this, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle);

            fp.redraw(panel, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle);

//            if(showSolution == true){
//                map.draw_solution(panel,);
//            }
    ...
    public int getPercentdone(){
        ...
        protected void setRobot(BasicRobot robot){
            this.robot = robot;
        }
        ...
        public void slowedDownRedraw() {
            System.out.println("Visited slowed down redraw");
            notifyViewerRedraw() ;
            try {
        ...
        private void rotateStep() {
            System.out.println("reached rotateStep");
        ...
        synchronized public void rotate(int dir) {
            System.out.println("reached rotate");
...
//                            notifyViewerRedraw() ; // seems useless but it is necessary to make the screen update
         ...
         */
        public void setState(StateGUI newState) {
...
//                    Constants.STEP_SIZE, seencells, 10, this)) ;
            ...
            System.out.println("SWITCHED TO PLAYING SCREEN CALLED");
            //ADDED 10/17
            if (driver != null && robot != null){
                System.out.println("Driver and robot are NOT null");
...
//                }
        ...
        public void updateProgress(int percentage) {
            ...
            if (percentdone < percentage && percentage <= 100) {
                ...
                GeneratingActivity.handlerProg.sendMessage(Message.obtain(GeneratingActivity.handlerProg, percentdone));
                Log.v("Title: progBar", "message sent");
