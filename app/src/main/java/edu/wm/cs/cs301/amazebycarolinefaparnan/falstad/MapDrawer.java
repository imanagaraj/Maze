...
public class MapDrawer extends DefaultViewer  {
	...
	public void redraw(MazeController controller, StateGUI state, int px, int py,
		...
		if (null != controller) {
			if (controller.isInMapMode()) {
				Log.v("drawing map", "isInMapMode");
				//wrapper = controller.panel;
				//wrapper = new MazePanel(controller);
				wrapper = controller.getPanel();
				draw_map(px, py, walk_step, view_dx, view_dy, controller.isInShowMazeMode(), controller.isInShowSolutionMode()) ;
	...
	public void draw_map( int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		...
		//wrapper.setGraphics(this.controller.panel.getBufferGraphics());r
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				//wrapper.setGraphicsColor(seencells.hasWall(x,y, CardinalDirection.North) ? "white" : "gray");
				wrapper.setColor(seencells.hasWall(x,y,CardinalDirection.North) ? Color.WHITE : Color.GRAY);
				if ((seencells.hasWall(x,y, CardinalDirection.North) || showMaze) && theCondition)
					//wrapper.getGraphics().drawLine();
				wrapper.drawLine(nx1, ny1, nx2, ny1);
				...
				//wrapper.setGraphicsColor(seencells.hasWall(x,y, CardinalDirection.West) ? "white" : "gray");
				wrapper.setColor(seencells.hasWall(x,y,CardinalDirection.West) ? Color.WHITE : Color.GRAY);
				if ((seencells.hasWall(x,y, CardinalDirection.West) || showMaze) && theCondition)
					//wrapper.getGraphics().drawLine(nx1, ny1, nx1, ny2);
				wrapper.drawLine(nx1, ny1, nx2, ny1);
			}
		
		if (showSolution) {
			draw_solution(wrapper, offx, offy, px, py) ;
		}
	}
	/**
	 ...
	 * @param view_dx to draw on
	 */
	public void draw_currentlocation(int view_dx, int view_dy) {
		Log.v("MapDrawer", "draw_current_location");
		//wrapper.setGraphics(this.controller.panel.getBufferGraphics());
		...
		wrapper.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		// now the drawing
		wrapper.drawLine(ctrx, ctry, aptx, apty);
		wrapper.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		wrapper.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	 ...
	 * @param wrapper to draw lines on
	 * @param offx
	 * @param offy
	 */
	public void draw_solution(MazePanel wrapper, int offx, int offy, int px, int py) {
		...
		while (distance > 1) {
			...
			wrapper.drawLine(nx1, ny1, nx2, ny2);
