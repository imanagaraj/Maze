...
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.BSPLeaf;
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.BSPNode;
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.BSPBranch;
...
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.MazeBuilder;
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.Seg;
import edu.wm.cs.cs301.amazebycarolinefaparnan.generation.Wall;
...
//	private Graphics2D gc ; // the graphics object for the buffer image this class draws on
	 ...
	 * @param bsp_root
	 */
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells seencells, BSPNode bsp_root, MazeController controller) {
		// store given parameter values
        System.out.println("Reached FirstPersonDrawer");
		view_width = width ;
		view_height = height ;
		...
		//this.wrapper = new GraphicsWrapper();
	}
	 ...
	 * @param wrapper
	 * graphics handler for the buffer image that this class draws on
	 ...
	 */
	@Override
	public void redraw(MazePanel wrapper, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		System.out.println("Redraw in FPD");
...
//		this.wrapper.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
//				RenderingHints.VALUE_ANTIALIAS_ON);
//
//		this.wrapper.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
...
//				RenderingHints.VALUE_INTERPOLATION_BILINEAR);
		...
		//wrapper.drawBackground(this.view_width, this.view_height);
		...
		traverse_node_ct = traverse_ssector_ct = drawrect_ct = drawrect_late_ct = drawrect_segment_ct = 0;
		//
	 ...
	 * @param wrapper
	 */
	public void drawBackground(MazePanel wrapper) {
		//wrapper.setGraphicsColor("black");
		wrapper.setColor(Color.BLACK);
		//gc.setColor(Color.black);
		wrapper.fillRect(0, 0, view_width, view_height/2);
	...
	private boolean boundingBoxIsVisible(BSPNode b) {
		...
		// constraint: x1 <= x2
		wrapper.setPoint(x1, x2);
		//Point p = new Point(x1, x2);
		return (rset.intersect(wrapper));
	}
	...
	private void drawSegment(Seg seg) {
		...
		int x1i = x1;
		int xd = x2-x1;
		wrapper.setColor(seg.col.getR(),seg.col.getG(),seg.col.getB());
		...
		while (x1i <= x2) {
			...
			wrapper.setPoint(x1i, x2);
			//Point p = new Point(x1i, x2);
			if (!rset.intersect(wrapper))
				break;
			x1i = wrapper.getPoint().x;
			int x2i = wrapper.getPoint().y;
			...
			wrapper.fillPolygon(xps, yps, 4);
			//wrapper.setColor(Color.BLACK);
			//wrapper.fillRect(0, 0, view_width, view_height/2);			//gc.fillPolygon(xps, yps, 4);
